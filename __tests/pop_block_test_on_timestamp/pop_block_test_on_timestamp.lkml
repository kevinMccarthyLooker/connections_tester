...
view: +order_items {
  ...
  dimension_group: created_using_datatype_timestamp {
    type: time
    # datatype: timestamp #ok
    # sql: cast(${TABLE}.created_at as timestamp) ;;
    # sql: cast('2020-01-01' as timestamp) ;;

    # datatype: date #ok
    # sql:cast('2020-01-01' as date);;

    # datatype: yyyymmdd #error: applies a date_trunk...
    # sql:'20200101';;

    datatype:epoch
    sql: from_unixtime(1556895150);;
  }
}
...
view: +order_items {#!Update to point to your view name (with the '+' making it a refinement).  That view's file must be included here, and then THIS file must be included in the explore
  ...
  dimension: selected_period_size {
    ...
    sql:{%if pop_support.period_size._parameter_value != 'Default'%}{{pop_support.period_size._parameter_value}}
        {% else %}
          {% if
created_using_datatype_timestamp_date._is_selected %}Day
          {% elsif
created_using_datatype_timestamp_month._is_selected %}Month
          {% else %}Year
          {% endif %}
  ...
  dimension: created_date_periods_ago_pivot {#!Update to match your base field name. This is generic sql logic (so you might expect it to be in pop_support template), but it is helpful to manifest this lynchpin pivot field here so we can create a dedicated pivot field in this specific date dimension's group label.
    label: "{% if _field._in_query%}Pop Period (created_using_datatype_timestamp {{selected_period_size._sql}}){%else%} Pivot for Period Over Period{%endif%}"#makes the 'PIVOT ME' instruction clear in the field picker, but uses a dynamic output label based on the period size selected
    group_label: "created_using_datatype_timestamp Date" #!Update this group label if necessary to make it fall in your date field's group_label
    can_filter: no
