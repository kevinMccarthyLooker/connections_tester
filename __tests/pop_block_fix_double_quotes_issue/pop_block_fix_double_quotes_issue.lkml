...
view: +order_items {
  ...
  dimension_group:another_date {
    type: time
    datatype: date
    sql:
    {% if _dialect._name == 'bigquery_standard_sql'%}
    cast(TIMESTAMP_TRUNC(cast(${created_raw} as timestamp), DAY) as date)
    {% else%}cast(${created_raw} as date)
    {%endif%}
    ;;
  }
}
...
view: pop_support {
  ...
  dimension: pop_sql_years_using_now  {type: date_time expression:  add_years(${periods_ago},${now_sql});;}#use looker expressions to get dialect specific sql for date add functions
  dimension: pop_sql_months_using_now {type: date_time expression:  add_months(${periods_ago},${now_sql});;}
  dimension: pop_sql_days_using_now   {type: date_time expression:  add_days(${periods_ago},${now_sql});;}
...
view: +order_items {#!Update to point to your view name (with the '+' making it a refinement).  That view's file must be included here, and then THIS file must be included in the explore
  ...
  # dimension_group: created {
    ...
    datatype: timestamp #our timezone conversion that we leverage results in a daily source datatype
#     sql:
#     {% assign now_converted_to_date_with_timezone_sql = "${pop_support.now_converted_to_date_with_tz_sql::date}" %}{% assign now_unconverted_sql = pop_support.now_sql._sql %}{%comment%}pulling in logic from pop support template, within which we'll inject the original sql. Use $ {::date} when we want to get looker to do conversions, but _sql to extract raw sql {%endcomment%}
...
#           {% assign tz_conversion_start  = now_converted_to_date_with_timezone_sql | split: now_unconverted_sql | first %}
#           {% assign tz_conversion_end    = now_converted_to_date_with_timezone_sql | split: now_unconverted_sql | last %}
#           {% assign tz_conversion_array    = now_converted_to_date_with_timezone_sql | split: now_unconverted_sql %}
#           {% assign pop_sql_wrapper_array = pop_sql_using_now | split: now_unconverted_sql %}

# {% if pop_support.periods_ago._in_query %}
# {% for part in pop_sql_wrapper_array %}
#   {{part}}
#   {% if forloop.last == false  %}
#     {% for tz_part in tz_conversion_array %}
#       {{tz_part}}{% if forloop.last == false  %}${EXTENDED}{%endif%}
#     {%endfor%}
#   {%endif%}
# {%endfor%}


#           {%else%}{{tz_conversion_start}}${EXTENDED}{{tz_conversion_end}}
#           {%endif%};;#wraps your original sql (i.e. ${EXTENDED}) inside custom pop logic, leveraging the parameterized selected-period-size-or-smart-default (defined below)
    sql:
    {% assign now_converted_to_date_with_timezone_sql = "${pop_support.now_converted_to_date_with_tz_sql::datetime}" %}{% assign now_unconverted_sql = pop_support.now_sql._sql %}{%comment%}pulling in logic from pop support template, within which we'll inject the original sql. Use $ {::date} when we want to get looker to do conversions, but _sql to extract raw sql {%endcomment%}
    {% assign selected_period_size = selected_period_size._sql | strip%}
    ...
    {% assign tz_conversion_array = now_converted_to_date_with_timezone_sql | split: now_unconverted_sql %}
    {% assign tz_conversion_array_size = tz_conversion_array | size%}
    {% assign pop_sql_array = pop_sql_using_now | split: now_unconverted_sql %}
    {% if pop_support.periods_ago._in_query %}
    {% for part in pop_sql_array %}{%comment%}parse the generic version of the pop sql and inject EXTENDED(potentially timezone converted){%endcomment%}
    {{part}}
    {% if forloop.last == false  %}
    {% assign size = tz_conversion_array | size%}
    {% if tz_conversion_array_size == 0 %}${EXTENDED}
    {%else%}
    {% for tz_part in tz_conversion_array %}
    {{tz_part}}{% if forloop.last == false  %}${EXTENDED}{%endif%}
    {%endfor%}
  ...
  dimension: selected_period_size {
    ...
    sql:{%if pop_support.period_size._parameter_value != 'Default'%}{{pop_support.period_size._parameter_value}}
        {% else %}
          {% if
another_date_date._is_selected %}Day
          {% elsif
another_date_month._is_selected %}Month
          {% else %}Year
          {% endif %}
...
test: pop_fields_dont_cause_error {
  explore_source: order_items {
    ...
    column: created_month {field:order_items.another_date_month}
    column: count {}
  }
